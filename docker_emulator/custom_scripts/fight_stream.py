...
def process_frame(obs, ffmpeg_process, step_count):
    try:
        logging.info(f"Step {step_count}: Processing observation")
        
        if isinstance(obs, gym.wrappers.frame_stack.LazyFrames):
            frame_stack = np.array(obs)
            frame_rgb = np.transpose(frame_stack[-3:], (1, 2, 0))
        else:
            frame_rgb = obs

        logging.info(f"Frame shape before resize: {frame_rgb.shape}")
        logging.info(f"Frame dtype before resize: {frame_rgb.dtype}")
        logging.info(f"Frame min/max before resize: {frame_rgb.min()}, {frame_rgb.max()}")

        frame_rgb = cv2.resize(frame_rgb, (1280, 720), interpolation=cv2.INTER_CUBIC)

        logging.info(f"Frame shape after resize: {frame_rgb.shape}")
        logging.info(f"Frame dtype after resize: {frame_rgb.dtype}")
        logging.info(f"Frame min/max after resize: {frame_rgb.min()}, {frame_rgb.max()}")

        frame_rgb = frame_rgb.astype(np.uint8)

        logging.info(f"Frame shape after uint8 conversion: {frame_rgb.shape}")
        logging.info(f"Frame dtype after uint8 conversion: {frame_rgb.dtype}")
        logging.info(f"Frame min/max after uint8 conversion: {frame_rgb.min()}, {frame_rgb.max()}")
    ...
    try:
        ...
        for round in range(args.num_rounds):
            ...
            while not (terminated or truncated) and step_count < max_steps:
                if step_count % frame_skip == 0:  # Changed from 'if not step_count % frame_skip == 0:'
                    if not process_frame(obs, ffmpeg_process, step_count):
