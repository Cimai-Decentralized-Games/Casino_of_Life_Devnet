...
export function ExplorerLink({
  ...
  const getExplorerUrl = (path: string) => {
    ...
    const networkParam = cluster.network === 'mainnet-beta' ? '' : `?cluster=${cluster.network}`;
    return `${baseUrl}${path}${networkParam}`;
  };
...
export function ClusterChecker({ children }: { children: ReactNode }) {
  ...
  if (query.isError || !query.data || !cluster) {
    return (
      <div className="alert alert-warning text-warning-content/80 rounded-none flex justify-center">
        <span>
          Error connecting to cluster <strong>{cluster?.name || 'Unknown'}</strong>
        </span>
        <button
...
export function ClusterUiSelect() {
  ...
  if (!cluster) {
    return null; // or return a loading state or default text
  }
...
export function ClusterUiTable() {
  ...
  if (!clusters || clusters.length === 0) {
    return <p>No clusters available.</p>;
  }
